...
int max_whites_captured(const vector<string> &grid) {
    ...
    unordered_map<int, int> captured_whites_count_by_knot;
    ...
    for (const auto &codes : g.groups()) {
        ...
        if (sz(empty_neighs) == 1) {
            const auto [ro, co] = *cbegin(empty_neighs);
            captured_whites_count_by_knot[ro * W + co] += sz(codes);
        }
    }

    int result{};
    for (const auto &[_, m] : captured_whites_count_by_knot) {
        result = max(result, m);
    }
    return result;
}
...
int main() {
    ...
    for (int i = 1; i <= t; ++i) {
        ...
        cout << "Case #" << i << ": " << max_whites_captured(grid) << '\n';
    }
