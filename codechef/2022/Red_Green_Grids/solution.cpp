...
vector<mint> factorials() {
    vector<mint> result(1000 * 1000 + 1, 1);
    for (int i = 2; i < sz(result); ++i) {
...
mint sum_of_scores(const vector<mint> &fact, const vector<vector<mint>> &nck,
...
int main() {
    ...
    while (t--) {
        ...
        cout << sum_of_scores(fact, nck, pnum, N, M).val() << '\n';
    }
